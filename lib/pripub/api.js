...
function PriPub(options) {
  ...
  this.binding = new pripub.binding.PriPub(this.pri, this.pub);
...
PriPub.prototype.encode = function encode(data, encoding) {
  if (!(data instanceof Buffer) || encoding) data = new Buffer(data, encoding);

  return this.binding.encode(data);
};

PriPub.prototype.decode = function decode(data, encoding) {
  if (!(data instanceof Buffer) || encoding) data = new Buffer(data, encoding);

  return this.binding.decode(data);
};
